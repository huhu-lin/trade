...
`config/supply_chain.yml` 目前定義三個獨立催化劑（AI 資料中心 capex、GLP-1 減重藥物擴產、
電網級儲能鋰化工需求）。每週執行時 `catalyst_detector.py` 會用各催化劑 driver/seed 節點
「當週真實」的營收/資本支出年增率算出熱度分數，`pipeline.py` 自動挑熱度最高者展開推薦
——**不是寫死選 AI**。目前 AI capex 熱度確實最高（多數週仍會選到它），但一旦其動能真的
降溫、且另一主題的財報動能超車，下一次執行會自動換題材，不需要改任何程式碼。
`tests/test_pipeline.py::test_catalyst_auto_rotates_away_from_ai_when_another_theme_heats_up`
用合成財報數據驗證了這個輪動機制。

要再擴充題材，只要在 `supply_chain.yml` 加一個新 catalyst + 有來源的 `verified: true` 邊、
並把節點加進 `universe_*.yml` 即可——熱度排名與雙引擎篩選會自動把它納入輪動候選。

> 目前**不會**做全市場即時新聞/公告掃描來自動生成全新主題：那需要用 LLM 從新聞或公告推論
> 供應鏈關係，而未經人工附來源查證的邊本來就被設計成「敘事假設」、不能影響信心分數（見
> 下方「反臆測設計」）。`supply_chain.py::propose_edge()` 已預留給 Claude 從財報提議新邊的
> 掛勾，但新邊進來一律 `verified:false`，需要人工附來源核實才會被採計——這是刻意的，避免
> 為了「更動態」而犧牲每個推薦背後真實可查證的因果鏈。
